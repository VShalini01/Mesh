#!/usr/bin/env python3
# Freeform gradient reconstruction with animation for optimisation process
import numpy as np
import cv2
...
def boundary2str(X, Y, G):
    """Convert boundary information to string for printing"""
    def colour2str(c):
        s = "("
...
def sample_vertex_colors_from_image(vertices, original_image):
    ...
    Returns:
        vertex_colors: Array of colors (N, 3) for each vertex in [0,1] range (like harmonic.py)
    """
    ...
    for vertex in vertices:
        ...
        x = max(0, min(x, w-1))
        y = max(0, min(y, h-1))
        # Sample color at vertex position and normalize to [0,1]
...
def compute_mesh_laplacian(vertices, vertex_colors, triangles):
    ...
    for channel in range(3):  # R, G, B channels
        # Apply Laplacian operator to this channel
...
def find_control_points_mesh_based(vertices, vertex_colors, triangles, top_k=3):
    ...
    Args:
        ...
        vertex_colors: Array of vertex colors (N, 3) in [0,1] range
        triangles: Array of triangle indices (M, 3)
        top_k: Number of control points to find per channel (default 3 like harmonic.py)
    ...
    # Step 1: Process each channel separately 
    ps_max_per_channel = []
    ps_min_per_channel = []
...
def process_canny_edge_image(edge_path, original_image_path):
    ...
    vertices, triangles = mesh_all_segments(boundaries, labels_img.shape, max_volume=500.0, min_angle=30.0)  # Less dense mesh like harmonic.py
    ...
    # Plot control points as black dots (like initialize.py)
    ...
    M.optimize_with_animate(vertex_colors, Boundary, Theta_Indx, alpha=5e-4, eps=1e-2, last_E=e, max_iter=67)
...
class Manifold:
    ...
    def dE_dY(self, g, Boundary, Theta_Indx):
        ...
        # Honestly, B is not needed. We can just use Lambda. But I am keeping it to follow the derivation in my
        # notebook. Matrix B defines the Jacobian of u w.r.t. Y, restricted to inner vertices. That is each column of B
        # is the Jacobian of u w.r.t. Y_i Extending B with identity of size K, we get the Jacobian of u w.r.t.
        # Y for all Y.
    ...
    def dE_dX(self, g, Boundary, Theta_Indx):
        ...
        def diff(i):
            Boundary_ = Boundary  # deepcopy(Boundary) avoiding deep copy at each iteration. Careful with push and pop
            X, Y = Boundary_
            assert i < len(X)
            x_i = X[i]
            ds = []
...
def main():
    ...
    if len(sys.argv) != 3:
        print("Usage: python freeform2.py <canny_edge_image_path> <original_image_path>")
        print("Example: python freeform2.py edge-1.png org_img/image.png")
        sys.exit(1)
